# STUDY 3 NOTE ("People" generic)
This folder is a copy of Study 2 with ONE manipulation change: the group-level
speech says "People (must) like to X" instead of "Wugs (must) like to X".
Data rows carry `study = "study3"`. All columns below are unchanged.
Before running children: regenerate the 22 group-speech clips with
`generate_study3_audio.py` (they still say "Wugs" until you do!) and then
run `equalize_volume.py`.
