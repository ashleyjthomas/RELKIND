...
#   Boss:                predict < .5 (choose group, i.e. less likely individual)
chance_tests <- pp |>
  group_by(questionType, epistemic) |>
  summarise(
    n       = n(),
    mean_p  = mean(p_chose_indiv),
    alt     = if (first(questionType) == "close") "greater" else "less",
    t_stat  = if (n() > 1) t.test(p_chose_indiv, mu = 0.5, alternative = first(alt))$statistic else NA,
    df      = if (n() > 1) t.test(p_chose_indiv, mu = 0.5, alternative = first(alt))$parameter else NA,
    p_value = if (n() > 1) t.test(p_chose_indiv, mu = 0.5, alternative = first(alt))$p.value   else NA,
    .groups = "drop"
  )
cat("\nOne-sided one-sample t-tests vs chance (0.5):\n")
cat("  close: predict > .5;  boss: predict < .5\n")
print(chance_tests)
write_csv(chance_tests, "tables/chance_tests_per_cell.csv")

# Trial-level binomial sanity check
binom_tests <- dat_full |>
  group_by(questionType, epistemic) |>
  summarise(
    n_trials = n(),
    n_indiv  = sum(chose_individual == 1),
    p        = n_indiv / n_trials,
    p_value  = if (first(questionType) == "close")
                  binom.test(n_indiv, n_trials, 0.5, alternative = "greater")$p.value
               else
                  binom.test(n_indiv, n_trials, 0.5, alternative = "less")$p.value,
    .groups  = "drop"
  )
cat("\nTrial-level binomial tests vs chance:\n"); print(binom_tests)
